    ...
    const ComboManager = {
        ...
        async applyDiscount(id, name, savings, symbol, position, ev) {
            ...
            try {
                ...
                const result = data.result || data;
                const t = result.translations || {};
                const sym = result.currency_symbol || symbol || '€';
                const pos = result.currency_position || position || 'after';
                if (result.success) {
                    ...
                    $('.simple-combo-alert h4').html(`✅ ${applied}`);
                    $('.simple-combo-alert p').html(`<strong>${result.message}</strong>`);
                    ...
                    $('.simple-combo-alert h4').html(`⚠️ ${warn}`);
                    $('.simple-combo-alert p').html(`<strong>${result.error}</strong>`);
                    $('.simple-combo-alert button').remove();
                ...
                $('.simple-combo-alert h4').html(`❌ ${connErr}`);
                $('.simple-combo-alert p').html(`<strong>Error: ${error.message}</strong>`);
                $('.simple-combo-alert button').remove();
                setTimeout(() => $('.simple-combo-alert').fadeOut(), 5000);
            }
        },
