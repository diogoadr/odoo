...
class ComboController(http.Controller):
    ...
    def _combo_requirements(self, combo):
        return {
            item.product_id.id: getattr(item, 'quantity', 1) or 1
    ...
    @http.route("/shop/check_combos", type="json", auth="public", website=True)
    ...
    @http.route("/shop/apply_discount/<int:combo_id>", type="json", auth="public", website=True)
    def apply_discount(self, combo_id):
        ...
        if max_times <= 0:
            ...
            return {
                "success": False,
                "error": _("Insufficient quantity to apply combo"),
                "currency_symbol": currency.symbol,
                "currency_position": currency.position,
            }
        ...
        total_savings = (total_price - combo.base_price) * max_times
        message = _(
            "%(count)sx Pack \"%(name)s\" applied - Total savings: %(amount)s"
        ) % {
            "count": max_times,
            "name": combo.name,
            "amount": self._format_currency(total_savings, currency),
        }
    ...
    @http.route("/shop/recalculate_combos", type="json", auth="public", website=True)
